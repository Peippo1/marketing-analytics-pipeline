...
def cleaned_data():
    fixture_path = (
        Path(__file__).resolve().parent / "fixtures" / "clean_marketing_sample.csv"
    )
...
def test_no_null_values(cleaned_data):
    assert cleaned_data.isnull().sum().sum() == 0, (
        "There are null values in the cleaned data."
    )
...
def test_response_binary(cleaned_data):
    ...
    assert unique_values.issubset({0, 1}), (
        f"Unexpected values in 'Response': {unique_values}"
    )
